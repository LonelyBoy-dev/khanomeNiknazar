@php $table = base64_encode('post_categories'); @endphp
<style>
...
<div class="card-body">
    <div class="row operation">
        ...
        <form style="display: contents;" method="get" action="/admin/trashed/postCategories">
                ...
                <div class="table100">
                    <table class="table table-bordered" data-page="<?= $pageNum+1?>">
                        ...
                        @foreach($items as $item)
                            ...
                            <tr id="item{{$item->id}}">
                                <td class="1">
                                    <input type="checkbox" name="delete" id="check_cat{{$item->id}}" value="{{$item->id}}" class="filled-in chk-col-light-blue checkBox">
                                    <label style="top: 8px" for="check_cat{{$item->id}}"></label>
                                </td>
                                <td>{{$item->title}}</td>
                                <td>{{$item->slug}}</td>
